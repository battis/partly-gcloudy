...
export type Options = {
   ...
   * Paging may be applied before or after --filter and --limit depending on the
   * service.
   */
  pageSize?: 'unlimited' | number;
...
export async function list({
  ...
  const flags = {
    ...
    'sort-by': sortBy ? sortBy.toString() : undefined,
    ...options
  };
  return gcloud<Revision[]>('run revisions list', {
    flags
  });
}
